	...
	return (
		<>
			<nav
				...
				className={`${
					sidebarIsOpen ? "absolute inset-y-0 right-0 translate-x-0 shadow-lg" : "absolute right-0 translate-x-full"
			...
			>
				...
				<div className="flex-1 space-y-4 px-3 py-3">
					<div className="flex flex-col space-y-1">
						...
						<Input placeholder="Slug" value={draftState.slug} onChange={handleSlugChange} />
					</div>
			...
			<div
				className={`fixed inset-0 z-20 bg-black bg-opacity-5 transition duration-200 ease-in-out dark:bg-opacity-50 ${
					sidebarIsOpen ? "pointer-events-auto opacity-100" : "pointer-events-none opacity-0"
				}`}
