				...
				<Detail.Header>
					<div className="flex items-center space-x-4 pb-2">
						...
						<div className="flex space-x-1">
							<Link href={`/u/${question.author?.name}`} className="inline-flex space-x-1">
							...
							<p className="text-quaternary line-clamp-1 leading-snug" title={createdAt.raw}>
								{createdAt.formatted}
							</p>
						</div>
					</div>
					...
					{question.audioUrl && (
						<div className="py-4">
							<AudioPlayer src={question?.audioUrl} isRecorder={false} id={question.id} waveform={question?.waveform} />
						</div>
					)}
